...
主循环负责收获 / 卖出 / 种植，常驻模式同时还会跑一个独立的定时器，专门负责"一键务农"和心跳通知：

- **每 10 分钟**检查一次页面上的 `一键务农` 按钮
  - 按钮存在并点击成功 → 等待 3 秒后发送 `🌾 一键务农已执行` 通知，本轮不再发心跳
  - 按钮缺失或点击失败 → 静默处理，不发通知（除非正好命中心跳轮）
- **每 60 分钟**发送一次心跳 `🟢 心跳 Ping`
  - 用计数器实现：每 6 次定时检查（6 × 10 分钟 = 60 分钟）为一次心跳轮
  - 仅当心跳轮且本轮没有发送过务农通知时，才发心跳

这个定时器只负责"轻量检测 + 通知"，不会触发收获、卖出、种植等主流程操作。
